...
//  IOS_CommonEditor
//
//  Centralized mutations for Layers v2. All operations normalize active ordering.
//
...
public struct LayerReducer {
    ...
    public mutating func toggleLock(id: Int) {
        if var node = tree.nodes[id] {
            node.isLocked.toggle()
            tree.nodes[id] = node
        }
    }

    public mutating func toggleHidden(id: Int) {
        if var node = tree.nodes[id] {
    ...
    public mutating func toggleExpanded(id: Int) {
        if var node = tree.nodes[id] {
    ...
    public mutating func select(id: Int) {
        for key in tree.nodes.keys {
            if var node = tree.nodes[key] {
                node.isSelected = (key == id)
                tree.nodes[key] = node
            }
        }
    }
    ...
    private mutating func normalize(parentId: Int) {
        ...
        // rewrite orders to be dense 0...n-1
