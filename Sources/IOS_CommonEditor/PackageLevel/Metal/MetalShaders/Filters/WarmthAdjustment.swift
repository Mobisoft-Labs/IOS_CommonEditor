...
final class WarmthAdjustment {
    var warmthIntensity: Float = 0.0
    ...
    init(library: MTLLibrary) throws {
        ...
        let function = try library.makeFunction(name: "WarmthAdjustment", constantValues: constantValues)
    ...
    func encode(source: MTLTexture,
        ...
        encoder.label = "WarmthAdjustment"
        encoder.setTexture(source, index: 0)
        encoder.setTexture(destination, index: 1)
        encoder.setBytes(&self.warmthIntensity, length: MemoryLayout<Float>.stride, index: 0)
