...
final class SharpnessAdjustment {
    var sharpnessIntensity: Float = 0.0
    ...
    init(library: MTLLibrary) throws {
        ...
        constantValues.setConstantValue(&self.deviceSupportsNonuniformThreadgroups,
                                        type: .bool,
                                        index: 0)
        let function = try library.makeFunction(name: "SharpnessAdjustment",
    ...
    func encode(source: MTLTexture,
        ...
        encoder.label = "SharpnessAdjustment"
        encoder.setTexture(source, index: 0)
        encoder.setTexture(destination, index: 1)
        encoder.setBytes(&self.sharpnessIntensity,
