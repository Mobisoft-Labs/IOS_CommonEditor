...
final class VibranceAdjustment {
    var vibranceIntensity: Float = 0.0
    ...
    init(library: MTLLibrary) throws {
        ...
        constantValues.setConstantValue(&self.deviceSupportsNonuniformThreadgroups,
                                        type: .bool,
                                        index: 0)
        let function = try library.makeFunction(name: "VibranceAdjustment",
    ...
    func encode(source: MTLTexture,
        ...
        encoder.label = "VibranceAdjustment"
        encoder.setTexture(source, index: 0)
        encoder.setTexture(destination, index: 1)
        encoder.setBytes(&self.vibranceIntensity,
