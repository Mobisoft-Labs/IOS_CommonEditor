...
final class ShadowsAdjustment {
    ...
    init(library: MTLLibrary) throws {
        ...
        constantValues.setConstantValue(&self.deviceSupportsNonuniformThreadgroups,
                                        type: .bool,
                                        index: 0)
        let function = try library.makeFunction(name: "ShadowsAdjustment",
    ...
    func encode(source: MTLTexture,
        ...
        encoder.label = "ShadowsAdjustment"
        encoder.setTexture(source, index: 0)
        encoder.setTexture(destination, index: 1)
        encoder.setBytes(&self.shadowIntensity,
