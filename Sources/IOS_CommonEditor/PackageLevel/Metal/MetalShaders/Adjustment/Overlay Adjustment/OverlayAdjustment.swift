...
//  VideoInvitation
//
//  Created by HKBeast on 08/09/23.
//
...
//  VideoInvitation
//
//  Created by HKBeast on 07/09/23.
//
...
final class OverlayAdjustment {
    ...
    init(library: MTLLibrary) throws {
        ...
        constantValues.setConstantValue(&self.deviceSupportsNonuniformThreadgroups,
                                        type: .bool,
                                        index: 0)
        let function = try library.makeFunction(name: "OverlayAdjustment",
                                                constantValues: constantValues)
        self.pipelineState = try library.device.makeComputePipelineState(function: function)
    }
    
    func encode(input1: MTLTexture,input2:MTLTexture,
                destination: MTLTexture,
                in encoder: MTLComputeCommandEncoder) {
//        guard let encoder = commandBuffer.makeComputeCommandEncoder()
//        else { return }
        encoder.label = "OverlayAdjustment"
        encoder.setTexture(input1,
                           index: 0)
        encoder.setTexture(input2,
                           index: 1)
        encoder.setTexture(destination,
                           index: 2)
        
        encoder.setBytes(&self.overlayOpacity,
